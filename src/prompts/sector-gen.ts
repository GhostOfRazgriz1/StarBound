...
export function buildSectorPreviewPrompt(runState: RunState): string {
  ...
  const lines = [
    ...
    '',
    `Current ship fuel: ${runState.ship.fuel}/${runState.ship.maxFuel}`,
    '',
    ...
    '- riskLevel: 1-5',
    '- interestLevel: 1-5',
    '- distance: 1-3 (1=nearby, 2=moderate, 3=far). Further sectors cost more fuel but tend to have better rewards.',
    '- encounterType: one of "civilization", "derelict", "anomaly", "pirate", "trader", "quiet"',
    '',
    'Guidelines:',
    '- Offer variety — different encounter types AND distances across the options',
    '- At least one option should be close (distance 1), one further (distance 2-3)',
  ...
  switch (type) {
    ...
    case 'trader':
      return [
        ...
        '  slot must be: "weapons"|"shields"|"engine"|"module"',
